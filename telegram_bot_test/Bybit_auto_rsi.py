...
#파일 경로입니다.
dolpha_type_file_path = "/var/autobot/BybitRSIDolPaCoinX.json"
try:
...
#파일 경로입니다.
change_value_file_path = "/var/autobot/BybitRSIchangeValueX.json"
try:
    #이 부분이 파일을 읽어서 리스트에 넣어주는 로직입니다. 
...
#영상에 없지만 스탑로스 주문이 가끔 사라진다는 걸 보완하기 위해 스탑로스 가격을 저장해두기 위한 저장소
StopLossDict = dict()

#파일 경로입니다.
stop_loss_file_path = "/var/autobot/BybitRSIstoplossX.json"
try:
    #이 부분이 파일을 읽어서 리스트에 넣어주는 로직입니다. 
    with open(stop_loss_file_path, 'r') as json_file:
        StopLossDict = json.load(json_file)
            ...
            if myBybit.CheckCoinInList(TopCoinList,ticker) == True or myBybit.CheckCoinInList(DolPaCoinList,ticker) == True:
                ...
                if myBybit.CheckCoinInList(DolPaCoinList,ticker) == True:
                    ...
                    if abs(amt_s) == 0 and abs(amt_b) == 0:
                        ...
                        #봇 도는 사이 남은 주문이 체결 될 수도 있으므로 1번 더 체크 한다                        
                        #잔고 데이타 가져오기 
                        balances2 = bybitX.fetch_positions(None, {'type':'Future'})
                        time.sleep(0.1)
                        ...
                        if abs(amt_b) > 0:
                            ...
                            #영상과는 다르게 아래처럼 활용합니다! 값이 아니라 비율로 구한 값을 활용하기에 아래처럼 씁니다 499번째 라인을 체크하세요!
                            target_price = entryPrice_b * (1.0 + ChangeValueDict[Target_Coin_Ticker])
                            ...
                            #######################################################################################################
                            #영상에 없는 방어 로직!!!
                            #현재 가격이 스탑로스보다 아래인데 스탑로스를 하지 않았다면 어떤 이유에서 스탑로스가 사라진 경우이므로 이땐 시장가로 종료!!!
                            if StopLossDict[Target_Coin_Ticker] > coin_price:
                                print(bybitX.create_order(Target_Coin_Ticker, 'market', 'sell', abs(amt_b), None,{'reduce_only': True,'close_on_trigger':True}))

                            
                            #익절 가격보다 현재가격이 높은데 아직 포지션이 있다? 시장가로 종료!!
                            if target_price < coin_price:
                                print(bybitX.create_order(Target_Coin_Ticker, 'market', 'sell', abs(amt_b), None,{'reduce_only': True,'close_on_trigger':True}))
                        ...
                        if abs(amt_s) > 0:
                            ...
                            #영상과는 다르게 아래처럼 활용합니다! 값이 아니라 비율로 구한 값을 활용하기에 아래처럼 씁니다 499번째 라인을 체크하세요!
                            target_price = entryPrice_s * (1.0 - ChangeValueDict[Target_Coin_Ticker])
                            ...
                            #######################################################################################################
                            #영상에 없는 방어 로직!!!
                            #현재 가격이 스탑로스보다 위인데 스탑로스를 하지 않았다면 어떤 이유에서 스탑로스가 사라진 경우이므로 이땐 시장가로 종료!!!
                            if StopLossDict[Target_Coin_Ticker] < coin_price:

                                print(bybitX.create_order(Target_Coin_Ticker, 'market', 'buy', abs(amt_s), None,{'reduce_only': True,'close_on_trigger':True}))
                                
                            #익절 가격보다 현재가격이 낮은데 아직 포지션이 있다? 시장가로 종료!!
                            if target_price > coin_price:
                    ...
                    if min % 5 == 0:
                        ...
                        #영상에서는 위에서 변동성을 구해 가격을 구했는데 가격이 아주 작은 코인의 경우 오작동의 여지가 있어
                        #아래처럼 비율로 구합니다 
                        #고가 / 저가 - 1.0 으로 고가가 1100원이고 저가가 1000원이라면 1.1이 나오는데 여기서 1.0을 빼면 0.1의 비율이 나오고 이걸 퍼센트로 바꾸면 10%가 됩니다.
                        change_value = ((float(df['high'][-2]) / float(df['low'][-2])) -1.0) * 0.5
          
                        #단 변동성이 현재 코인가격의 0.25%보다 작다면 맞춰준다!
                        if change_value < 0.0025:
                            change_value = 0.0025
                        ...
                        if up_first_point != 0 and up_second_point != 0:
                            ...
                            if abs(amt_b) == 0 and abs(amt_s) == 0 and df['close'][-(up_first_point)] < df['close'][-(up_second_point)] and len(DolPaCoinList) < CoinCnt:
                        ...
                        if down_first_point != 0 and down_second_point != 0:
                            #다이버전스 발견!
                            if abs(amt_b) == 0 and abs(amt_s) == 0 and df['close'][-(down_first_point)] > df['close'][-(down_second_point)] and len(DolPaCoinList) < CoinCnt:
                        ...
                        if IsLongDivergence == True and IsShortDivergence == False and abs(amt_b) == 0 and abs(amt_s) == 0:
                            ...
                            #영상과 다르게 아래처럼 활용합니다. 499번째 라인을 체크하세요!
                            target_price = coin_price * (1.0 + change_value)
                            ...
                            while TotalWater_Amt + Water_amt <= Max_Water_Amt: 
                                ...
                                #영상과 다르게 아래처럼 활용합니다. 499번째 라인을 체크하세요!
                                water_price = coin_price * (1.0 - ((change_value * 2.0) * float(i)))
                            ...
                            #영상과 다르게 아래처럼 활용합니다. 499번째 라인을 체크하세요!
                            stop_price = water_price * (1.0 - (change_value*2.0))
                            ...
                            ####################################################################
                            #영상에 없지만 스탑로스할 가격을 저장해 둔다! 혹시나 스탑로스가 사라질 경우 대비!!
                            StopLossDict[Target_Coin_Ticker] = stop_price
                            with open(stop_loss_file_path, 'w') as outfile:
                                json.dump(StopLossDict, outfile)
                            ####################################################################
                        ...
                        if IsShortDivergence == True and IsLongDivergence == False and abs(amt_b) == 0 and abs(amt_s) == 0:
