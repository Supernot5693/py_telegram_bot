...
#파일 경로입니다.
dolpha_type_file_path = "/var/autobot/BinanceRSIDolPaCoinX.json"
try:
...
#파일 경로입니다.
change_value_file_path = "/var/autobot/BinanceRSIchangeValueX.json"
try:
    #이 부분이 파일을 읽어서 리스트에 넣어주는 로직입니다. 
...
#파일 경로입니다.
stop_loss_file_path = "/var/autobot/BinanceRSIstoplossX.json"
try:
    #이 부분이 파일을 읽어서 리스트에 넣어주는 로직입니다. 
                ...
                if myBinance.CheckCoinInList(DolPaCoinList,ticker) == True:
                    ...
                    if abs(amt_s) == 0 and abs(amt_b) == 0:
                        ...
                        if abs(amt_b) > 0:
                            ...
                            #영상과는 다르게 아래처럼 활용합니다! 값이 아니라 비율로 구한 값을 활용하기에 아래처럼 씁니다 546번째 라인을 체크하세요!
                    ...
                    if min % 5 == 0:
                        ...
                        if down_first_point != 0 and down_second_point != 0:
                            ...
                            #다이버전스 발견!
                            if abs(amt_s) == 0 and abs(amt_b) == 0  and df['close'][-(down_first_point)] > df['close'][-(down_second_point)] and len(DolPaCoinList) < CoinCnt:
                        ...
                        if IsLongDivergence == True and IsShortDivergence == False and abs(amt_b) == 0 and abs(amt_s) == 0:
                            ...
                            #영상과 다르게 아래처럼 활용합니다. 546번째 라인을 체크하세요
                            target_price = data['price'] * (1.0 + change_value)
                            ...
                            while TotalWater_Amt + Water_amt <= Max_Water_Amt: 
                                ...
                                #영상과 다르게 아래처럼 활용합니다. 546번째 라인을 체크하세요
                                water_price = data['price'] * (1.0 - ((change_value * 2.0) * float(i)))
                                #############################################################
                            ...
                            #영상과 다르게 아래처럼 활용합니다. 546번째 라인을 체크하세요
                            stop_price = line_data['price'] * (1.0 - (change_value*2.0))
